...
export async function loadTelemetry(): Promise<DashboardTelemetry> {
  try {
    ...
    const artifact = normalizeTelemetryArtifact(rawArtifact);
...
function normalizeTelemetryArtifact(artifact: any): any {
  if (artifact?.metrics || artifact?.activeGeneration !== undefined) {
    return artifact;
  }
  const generations = artifact?.generations ?? [];
  const currentGeneration = artifact?.currentGeneration ?? generations.at(-1)?.generation ?? 0;
  const metrics = generations.map((point: any) => ({
    generation: Number(point.generation ?? 0),
    winRate: Number(point.winRate ?? 0),
    gamesPerSecond: 0,
    turnsPerSecond: 0,
    p95LatencyMs: 0,
    gpuUtilization: 0,
    evaluatedGames: Number(point.games ?? 0),
    sampledReplayGames: 0,
    modelActionCalls: 0,
    launchActions: 0,
    launchedShips: 0,
    captures: 0,
    ...
    sunDestroyedShips: 0,
    avgFleetSize: 0,
    avgLaunchActionsPerTurn: 0,
    avgLaunchedShipsPerTurn: 0,
    avgModelActionMs: 0,
    inferenceBatchCalls: 0,
    maxInferenceBatchSize: 0,
    simultaneousGames: 0,
    modelActionSeconds: 0,
    simulationStepSeconds: 0,
    evaluationSeconds: 0,
    replaySeconds: 0,
    replayWriteSeconds: 0,
    generationValidationGames: 0,
    generationValidationSeconds: 0,
    backpropSamples: 0,
    backpropModels: 0,
    backpropSeconds: 0,
    reproductionSeconds: 0,
    generationSeconds: 0,
  }));
  const models = (artifact?.models ?? []).map((model: any) => ({
    id: `V8-${String(model.modelId ?? model.generation ?? 0).padStart(MODEL_ID_PAD_WIDTH, '0')}`,
    parent: 'supervised',
    rating: Math.round(600 + Number(model.winRate ?? 0) * 100),
    wins: Number(model.wins ?? 0),
    draws: Number(model.draws ?? 0),
    losses: Number(model.losses ?? 0),
    games: Number(model.games ?? 0),
    captures: 0,
    fleetHits: 0,
    hitShips: 0,
    sunDestroyedFleets: 0,
    ...
    avgFleetSize: 0,
    mutation: 'v8',
    selected: Boolean(model.selected ?? false),
  }));
  const replayFrames = artifact?.replay?.frames?.map(hydrateFrame) ?? [];
  return {
    ...artifact,
    sourceMessage: artifact?.sourceMessage ?? 'normalized v8 telemetry artifact',
    activeGeneration: currentGeneration,
    runProfile: artifact?.runProfile ?? 'v8',
    trainingMode: artifact?.trainingMode ?? 'v8_action_slots',
    strictGameRules: artifact?.strictGameRules ?? true,
    episodeSteps: artifact?.episodeSteps ?? 500,
    expectedFramesPerGame: artifact?.expectedFramesPerGame ?? 501,
    populationSize: artifact?.populationSize ?? Math.max(1, models.length),
    eliteCount: artifact?.eliteCount ?? 1,
    gamesPerModel: artifact?.gamesPerModel ?? 0,
    replaysPerModel: artifact?.replaysPerModel ?? 0,
    generationReplayGameCount: artifact?.generationReplayGameCount ?? 0,
    playersPerGame: artifact?.playersPerGame ?? 2,
    simultaneousGames: artifact?.simultaneousGames ?? 0,
    inferenceBatchCalls: artifact?.inferenceBatchCalls ?? 0,
    maxInferenceBatchSize: artifact?.maxInferenceBatchSize ?? 0,
    mapSource: artifact?.mapSource ?? 'v8_dataset_or_arena',
    turnLoop: artifact?.turnLoop ?? 'v8_pipeline',
    fullReplay: artifact?.fullReplay ?? false,
    replayFrameStride: artifact?.replayFrameStride ?? 0,
    storedReplayGames: artifact?.storedReplayGames ?? (replayFrames.length ? 1 : 0),
    gamesPerSecond: artifact?.gamesPerSecond ?? 0,
    turnsPerSecond: artifact?.turnsPerSecond ?? 0,
    gpuUtilization: artifact?.gpuUtilization ?? 0,
    cpuWorkers: artifact?.cpuWorkers ?? 0,
    evalQueue: artifact?.evalQueue ?? 0,
    submissionGate: artifact?.submissionGate ?? 'v8 Kaggle submission manual gate',
    metrics,
    generationWinRates: artifact?.generationWinRates ?? [],
    models,
    frames: replayFrames,
    replayChunks: artifact?.replayChunks ?? [],
    replayGames: replayFrames.length
      ? [{
          generation: Number(artifact?.replay?.generation ?? currentGeneration),
          modelId: 'V8',
          gameIndex: Number(artifact?.replay?.gameIndex ?? 0),
          opponentId: 'artifact',
          reward: 0,
          frames: replayFrames,
        }]
