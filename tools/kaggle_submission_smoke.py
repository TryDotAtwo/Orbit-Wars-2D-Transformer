...
def main() -> None:
    ...
    for probe_step in [0, 2, 10, 50, 93]:
        ...
        actions = agent(obs)
        print(
            f"direct_probe_step={probe_step}; owned={[(p[0], p[5]) for p in owned]}; "
            f"action_count={len(actions)}; actions={actions[:8]}"
        )
    for index, state in enumerate(env.steps[-1]):
        print(
