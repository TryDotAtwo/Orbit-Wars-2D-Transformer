...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct HitFleetEvent {
    pub player: i32,
    pub fleet_id: i32,
    pub target_planet_id: i32,
    pub ship_count: i32,
}
...
pub struct SimulationStepEvents {
    ...
    pub hit_fleets: Vec<HitFleetEvent>,
    ...
    fn move_fleets(
        ...
        for fleet in self.fleets.iter().copied() {
            ...
            if let Some(planet) = hit_planet {
                ...
                events.hit_fleets.push(HitFleetEvent {
                    player: fleet.owner,
                    fleet_id: fleet.id,
...
mod tests {
    ...
    fn fleet_collision_captures_target() {
        ...
        assert_eq!(events.hit_fleets.len(), 1);
        assert_eq!(events.hit_fleets[0].target_planet_id, 2);
